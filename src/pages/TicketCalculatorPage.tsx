...
function TimelineRowView({ row, language }: { row: TicketTimelineRow; language: string }) {
  ...
  return (
    <div className="flex flex-col gap-3 border-b border-border pb-5 last:border-b-0 last:pb-0 sm:flex-row sm:gap-4">
      <div className="flex shrink-0 flex-row items-center gap-3 sm:w-32 sm:flex-col sm:text-center">
        {row.kind === 'event' && row.event ? (
          <>
            ...
            <div className="flex flex-col gap-1 sm:items-center">
              ...
              <span className="text-xs font-semibold text-foreground">{row.event.characterName}</span>
            </div>
          </>
        ) : (
        ...
        <span className="text-[10px] font-medium uppercase tracking-wide text-subtle">{formatDate(row.date, language)}</span>
      </div>
          ...
          <div className="flex flex-col gap-5 rounded-card border border-border bg-surface p-5 sm:p-6">
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <label className="flex flex-col gap-1.5">
                ...
                <input
                  ...
                  className="w-full rounded-xl border border-border bg-elevated px-3.5 py-2.5 text-sm text-foreground focus:border-accent/50 focus:outline-none focus:ring-2 focus:ring-accent/15"
                />
              </label>
              <label className="flex flex-col gap-1.5">
                <span className="text-xs font-semibold uppercase tracking-wide text-muted">
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <div className="flex flex-col gap-3 rounded-xl border border-border bg-elevated/40 p-3">
                  <span className="text-xs font-bold uppercase tracking-wide text-accent-secondary">
                    {t('ticketCalculator.form.currentBlackTickets')}
                  </span>
                  {groupedBlackSources.map(([timing, sources]) => (
                    <div key={timing} className="flex flex-col gap-2">
                      <span className="text-[11px] font-semibold uppercase tracking-wide text-subtle">
                        {t(RELEASE_TIMING_LABEL_KEYS[timing])}
                      </span>
                      {sources.map((source) => (
                        <label key={source.id} className="flex items-center gap-2.5 text-sm text-muted">
                          <input
                            type="checkbox"
                            checked={claimedIds.has(source.id)}
                            onChange={() => toggleClaimed(source.id)}
                            className="h-4 w-4 rounded border-border"
                          />
                          <span>
                            {t(source.labelKey)} <span className="text-subtle">(+{source.amount})</span>
                          </span>
                        </label>
                      ))}
                    </div>
                ...
                <div className="flex flex-col gap-3 rounded-xl border border-border bg-elevated/40 p-3">
                  <span className="text-xs font-bold uppercase tracking-wide text-accent-info">
                    {t('ticketCalculator.form.currentStkTickets')}
                  </span>
                  {groupedStkSources.map(([timing, sources]) => (
                    <div key={timing} className="flex flex-col gap-2">
                      <span className="text-[11px] font-semibold uppercase tracking-wide text-subtle">
                        {t(RELEASE_TIMING_LABEL_KEYS[timing])}
                      </span>
            ...
            <div className="flex flex-col gap-3">
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <div className="inline-flex w-fit items-center rounded-full border border-border bg-elevated p-0.5">
                  {SERVERS.map((server) => (
                    <button
                      key={server}
                      type="button"
                      ...
                      className={`h-8 rounded-full px-3.5 text-xs font-semibold transition-colors duration-200 ${
                        targetServer === server ? 'bg-accent-info text-canvas' : 'text-subtle hover:text-foreground'
                      }`}
                    >
                      {server}
                    </button>
              ...
              <div className="inline-flex w-fit items-center rounded-full border border-border bg-elevated p-0.5">
                {(['next-release', 'custom-date'] as TargetMode[]).map((mode) => (
                  <button
                    ...
                    className={`h-9 rounded-full px-4 text-xs font-semibold transition-colors duration-200 ${
                      targetMode === mode ? 'bg-accent-secondary text-canvas' : 'text-subtle hover:text-foreground'
                    }`}
                  >
